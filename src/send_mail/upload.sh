...
ENV=$1
BUCKET_NAME=$ENV-personal-website-ld4ab847
...
aws s3 cp $FILE_NAME s3://$BUCKET_NAME/lambda/$FILE_NAME

# 要件6. Lambda関数にアップロードする。関数バージョンも同時に作成する。
aws lambda update-function-code --function-name $FUNCTION_NAME --s3-bucket $BUCKET_NAME --s3-key lambda/$FILE_NAME --publish
